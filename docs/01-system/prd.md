...
> **Note on use case numbering:** This PRD uses a functional workflow taxonomy (UC1–UC4). The canonical ROI-based use case taxonomy (UC1–UC7) is defined in `02-roi/master-overview.md` and is the naming convention used across all other documentation.
...
**UC1:** Document change event detected → agent classifies change type → generates findings routed to relevant document owners

**UC2:** Risk file change event detected → agent flags hazard and risk control gaps

**UC3:** Document approved or baselined → agent generates pre-approval compliance summary

**UC4:** Document marked obsolete → agent verifies successor documentation exists
