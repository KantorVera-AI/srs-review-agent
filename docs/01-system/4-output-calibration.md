...
When triggered, the agent generates a reclassification finding with mandatory acknowledgement (see `5-finding-lifecycle.md`). The finding cites: the current IMDRF category, the specific SRS change that may affect it, and the classification criteria that the change potentially crosses.
